...
def get_potential_stocks():
    """
    量化策略：【主升浪·主力箱体吸筹与点火模型】
    核心逻辑：
    1. 资金面：近 5 天内至少 3 天主力净流入（资金持续潜伏）且总额达标。
    2. 形态面：近 15 天内股价被压制在窄幅箱体（振幅<25%），证明主力在控盘压价吸筹。
    3. 启动面：今日股价站稳 10 日均线之上（吸筹结束，开始点火）。
    4. 基础面：主板、非ST、低价、建仓级换手率。
    """
    try:
        # 1. 划定时间窗口：取近 15 个交易日（用于计算箱体）和近 5 个交易日（用于计算潜伏资金）
        ...
        logging.info(f"正在扫描全市场，时间窗口: {dates_15[0]} 至 {last_date}")
        ...
        # 箱体控盘条件：15天内上下振幅必须小于 25% (说明主力在刻意压盘，没有让股价起飞，还在收集筹码)
        ...
        df_basic = pro.daily_basic(trade_date=last_date, fields='ts_code,close,turnover_rate,total_mv')
        ...
        logging.info(f"🎯 主力吸筹量化模型扫描完毕！精选标的: {list(zip(stock_list, names))}")
        return stock_list
